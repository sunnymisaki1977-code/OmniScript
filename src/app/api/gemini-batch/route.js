...
export async function POST(req) {
  try {
    ...
    const { theme, stepIds, context } = body;
    ...
    const stepsToRun = typeof stepIds === 'string' 
      ? (stepIds === 'all' ? [1,2,3,4,5,6,7,8,9] : stepIds.split(',').map(Number))
      : Array.isArray(stepIds) ? stepIds : [];

    const validSteps = workflowSteps.filter(s => stepsToRun.includes(s.id));
    ...
    let masterPrompt = `你現在是頂尖的全域企劃 AI 助理。請針對主題「${theme}」一次性產出 ${validSteps.length} 個步驟的完整企劃內容。
    
【絕對要求】：
1. 你必須在腦海中依序完成這些步驟的推演。當後面的步驟要求參考前面的步驟時，請直接使用你剛剛產出的內容。
2. 你「必須且只能」回傳一個純 JSON 格式的字串，絕對不要包含任何 markdown 區塊標記 (如 \`\`\`json)，也不要有任何前後問候語。
3. JSON 的 key 必須精準命名為 ${validSteps.map(s => `"step${s.id}"`).join(", ")}。
